...
(function($) {
  ...
  new WOW().init();
    ...
    var v_count = typeof v_count !== 'undefined' ? v_count : '0';
            ...
            if (imagePos < topOfWindow + jQuery(window).height() && v_count === '0') {
                jQuery(function ($) {
                    ...
                    function count(options) {
                        ...
                        var toStr = ($this.attr('data-to') || '0').trim();
                        var decimals = 0;
                        if (toStr.indexOf('.') >= 0) {
                            decimals = toStr.split('.')[1].length;
                        }

                        options = $.extend({}, options || {}, $this.data('countToOptions') || {});

                        var speedAttr = parseInt($this.attr('data-speed'), 10);
                        if (!isNaN(speedAttr)) options.speed = speedAttr;

                        var toVal = parseFloat(toStr) || 0;
                        options.to = toVal;
                        options.decimals = decimals;

                        // فورمات نهائي: $196.9M
                        options.formatter = function (value, opts) {
                            var factor = Math.pow(10, opts.decimals || 0);
                            var truncated = opts.decimals ? (Math.trunc(value * factor) / factor) : Math.trunc(value);
                            var text = opts.decimals ? truncated.toFixed(opts.decimals) : String(truncated);
                            return text + 'M $';
                        };
